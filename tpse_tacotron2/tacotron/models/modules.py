...
class CBHG:
    def __init__(self, K, conv_channels, pool_size, projections, projection_kernel_size, n_highwaynet_layers,
                 highway_units, rnn_units, is_training, name=None):
        self.K = K
        ...
        with tf.variable_scope(self.scope):
            with tf.variable_scope('conv_bank'):
                ...
                conv_outputs = tf.concat(
                    [conv1d(inputs, k, self.conv_channels, tf.nn.relu, self.is_training, 0., 'conv1d_{}'.format(k)) for
                     k in range(1, self.K + 1)],
                    axis=-1
                )
...
def normalize(inputs,
              epsilon=1e-8,
              scope='transformer_layer_normalization',
              reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        beta = tf.Variable(tf.zeros(params_shape, dtype=tf.float32), dtype=tf.float32)
        gamma = tf.Variable(tf.ones(params_shape, dtype=tf.float32), dtype=tf.float32)
