...
class TacotronReferenceEncoderCell:
    def __init__(self, convolutional_layers, lstm_layer, style_token_layer, hparams, scope=None):
        self._convolutions = convolutional_layers
        self._cell = lstm_layer
        self._style_token_layer = style_token_layer
        self._hparams = hparams
        self._scope = scope if scope is not None else 'style_layer'
    ...
    def __call__(self, inputs, input_lengths=None, style_token_embedding=None):
        with tf.variable_scope(self._scope):
            ...
            # output[:,-1,:]: [batch_size,cell_size]
            # state_: [bathc_size,1,cell_size]
...
class TPSE_GST_Cell:
    def __init__(self, lstm_layer, fc_layer, scope='tpse_layer'):
        self._cell = lstm_layer
        self._fc_layer = fc_layer
        self._scope = scope if scope is not None else 'tpse_layer'

    def __call__(self, inputs, input_lengths=None):
        with tf.variable_scope(self._scope):
            # output[:, -1, :]: gru last output as summarier
            output, state = tf.nn.dynamic_rnn(self._cell, inputs, sequence_length=input_lengths, dtype=tf.float32)
            # [batch_size,cell_size]
            last_output = output[:, -1, :]
            # [batch_size,fc_size]
            fc_output = self._fc_layer(last_output)
        return fc_output
