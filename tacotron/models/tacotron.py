...
from tacotron.models.Architecture_wrappers import TacotronEncoderCell, TacotronDecoderCell, \
    TacotronReferenceEncoderCell, TPSE_GST_Cell
...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, linear_targets=None,
                   targets_lengths=None, gta=False, tp=True,
                    ...
                    if tp:  # text prediction
                        ...
                        if is_training or is_evaluating:
                            ...
                            self.style_embedding_table = tf.get_variable(
                                'style_token_embedding', [hp.tacotron_n_style_token, hp.embedding_dim],
                                dtype=tf.float32,
                            ...
                            # style_encoder_outputs: [batch_size,1,hp.tacotron_style_encoder_outputs_size]
                            ...
                            # [batch_size,hp.tacotron_style_encoder_outputs_size], this is just for tpse train
                            self.style_outputs = tf.squeeze(style_encoder_outputs, axis=1)
                            # [batch_size,seq_len,hp.tacotron_style_encoder_outputs_size]
    ...
    def add_tpse_loss(self):
        hp = self._hparams

        self.tower_tpse_loss = []
        total_tpse_loss = 0.
        ...
        for i in range(hp.tacotron_num_gpus):
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
                with tf.variable_scope('tpse_loss') as scope:
                    # Compute the regularization weight
                    if hp.tacotron_scale_regularization:
                        reg_weight_scaler = 1. / (2 * hp.max_abs_value) if hp.symmetric_mels else 1. / (
                            hp.max_abs_value)
        ...
        for i in range(hp.tacotron_num_gpus):
            total_tpse_loss += self.tower_tpse_loss[i]

        self.tpse_loss = total_tpse_loss / hp.tacotron_num_gpus
        ...
        for i in range(hp.tacotron_num_gpus):
            ...
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
                ...
                with tf.variable_scope('optimizer') as scope:
                    ...
                    # filter some gradients
                    gradients_ = []
                    for gradient in gradients:
                        # filter tpse_layer variables
                        if (gradient[0] is None) or (gradient[1] is None):
                            continue
                        if 'tpse_layer' in gradient[0].name or 'tpse_layer' in gradient[1].name:
                            continue
    ...
    def add_tpse_optimizer(self, global_step):
        '''Adds optimizer. Sets "gradients" and "optimize" fields. add_loss must have been called.
        Args:
            global_step: int32 scalar Tensor representing current global step in training
        '''
        ...
        with tf.device(grad_device):
            with tf.variable_scope('tpse_optimizer') as scope:
                if hp.tacotron_decay_learning_rate:
                    learning_rate = self._learning_rate_decay(hp.tacotron_initial_learning_rate, global_step)
                else:
                    learning_rate = tf.convert_to_tensor(hp.tacotron_initial_learning_rate)

                optimizer = tf.train.AdamOptimizer(learning_rate, hp.tacotron_adam_beta1,
                                                   hp.tacotron_adam_beta2, hp.tacotron_adam_epsilon)
        ...
        for i in range(hp.tacotron_num_gpus):
            ...
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
                # agg_loss += self.tower_loss[i]
                with tf.variable_scope('tpse_optimizer') as scope:
                    gradients = optimizer.compute_gradients(self.tower_tpse_loss[i])
                    # filter some gradients
                    gradients_ = []
                    for gradient in gradients:
                        # when train tpse, filter embedding, encoder, style_layer(gst) gradients
                        if (gradient[0] is None) or (gradient[1] is None):
                            continue
                        ...
                        if 'encoder' in gradient[0].name or 'encoder' in gradient[1].name:
                            continue
                        if 'style_layer' in gradient[0].name or 'style_layer' in gradient[1].name:
                            continue
                        gradients_.append(gradient)
                    gradients = gradients_
                    tower_gradients.append(gradients)
        ...
        with tf.device(grad_device):
            ...
            # https://github.com/tensorflow/tensorflow/issues/1122
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                self.tpse_optimize = optimizer.apply_gradients(zip(clipped_gradients, vars),
                                                               global_step=global_step)
