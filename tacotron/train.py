...
def add_train_stats(model, hparams):
    with tf.variable_scope('stats') as scope:
        ...
        tf.summary.scalar('tpse_loss',model.tpse_loss)
...
def model_train_mode(args, feeder, hparams, global_step):
    with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
        ...
        model.add_tpse_loss()
        model.add_tpse_optimizer(global_step)
    ...
    with tf.Session(config=config) as sess:
        try:
            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                step, loss, opt, tpse_loss, tpse_opt = sess.run(
                    [global_step, model.loss, model.optimize, model.tpse_loss, model.tpse_optimize])
